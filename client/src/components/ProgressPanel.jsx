  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-sm">
          ...
          return (
            <div key={s.key} className="flex flex-1 items-center">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-medium ${isActive ? 'text-brand-700 dark:text-brand-300' : isDone ? 'text-brand-600 dark:text-brand-400' : 'text-slate-400 dark:text-slate-500'}`}>
                  {s.label}{skipped && <span className="ml-1 text-[10px] text-slate-300 dark:text-slate-600">(off)</span>}
                </span>
              </div>
              {i < STEPS.length - 1 && <div className={`mx-2 h-0.5 flex-1 rounded ${i < cur || done ? 'bg-brand-200' : 'bg-slate-100 dark:bg-slate-800'}`} />}
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div>
          <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
            <span>Collected</span>
            <span className="tabular text-slate-700 dark:text-slate-200"><AnimatedNumber value={state.counts.kept} /> / {state.target}</span>
          </div>
        ...
        <div>
          <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="tabular text-slate-700 dark:text-slate-200"><AnimatedNumber value={state.counts.rawSeen} /></span>
          </div>
        ...
        <div>
          <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="tabular text-slate-700 dark:text-slate-200">
              {enrichOn
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 dark:border-slate-800 pt-3">
        <div className="truncate text-xs text-slate-500 dark:text-slate-400">{state.ticker || '—'}</div>
        <div className="flex shrink-0 items-center gap-2">
          {state.notices?.length > 0 && (
            <span title={state.notices.slice(-6).map((n) => n.message).join('\n')}
              className="rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 px-2 py-1 text-xs font-medium text-amber-700 dark:text-amber-400">
          ...
          {state.rejected?.length > 0 && (
            <button onClick={() => setTab(tab === 'rejected' ? null : 'rejected')}
              ...
              className="rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-2 py-1 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">
              {state.counts.skippedIrrelevant} filtered out {tab === 'rejected' ? '▾' : '▸'}
            </button>
          )}
          {state.errors.length > 0 && (
            <button onClick={() => setTab(tab === 'errors' ? null : 'errors')}
              className="rounded-md border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/40 px-2 py-1 text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-100">
      ...
      {tab === 'rejected' && (
        <div className="mt-2 max-h-48 space-y-1 overflow-y-auto rounded-lg bg-slate-50 dark:bg-slate-800 p-2 text-xs">
          <div className="pb-1 text-[11px] text-slate-400 dark:text-slate-500">
            These didn’t match your niche, so they didn’t use up your target.
          </div>
          {state.rejected.slice().reverse().map((r, i) => (
            <div key={i} className="flex items-baseline gap-2 text-slate-600 dark:text-slate-300">
              <span className="w-7 shrink-0 tabular text-right text-slate-400 dark:text-slate-500">{r.score ?? '—'}</span>
              <span className="truncate font-medium">{r.name || '(unnamed)'}</span>
              <span className="truncate text-slate-400 dark:text-slate-500">{r.reason}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === 'errors' && (
        <div className="mt-2 max-h-40 space-y-1 overflow-y-auto rounded-lg bg-red-50/60 dark:bg-red-950/30 p-2 text-xs">
      ...
      {state.nicheProfile && Object.keys(state.nicheProfile).length > 0 && (
        <div className="mt-2 flex flex-wrap items-center gap-1.5 border-t border-slate-100 dark:border-slate-800 pt-2">
          <span className="text-[10px] uppercase tracking-wide text-slate-400 dark:text-slate-500">Niche learned:</span>
          {Object.entries(state.nicheProfile).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([c, n]) => (
            <span key={c} className="rounded bg-brand-50 dark:bg-brand-950/40 px-1.5 py-0.5 text-[10px] text-brand-700 dark:text-brand-300">
