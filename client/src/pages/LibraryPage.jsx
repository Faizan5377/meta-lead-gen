    ...
    <>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-60 rounded-xl border border-slate-200 dark:border-slate-700 py-2 pl-8 pr-3 text-sm shadow-sm focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900"
            />
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 space-y-3 px-5 py-5 lg:px-7">
        <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
          ...
          <span><b className="tabular text-slate-800 dark:text-slate-100">{totalAds.toLocaleString()}</b> ads collected</span>
          <span className={stats?.mode === 'supabase' ? 'text-brand-600 dark:text-brand-400' : 'text-amber-600'}>
            {stats?.mode === 'supabase' ? 'synced to Supabase' : 'stored locally only'}
          </span>
        </div>

        {error && (
          <div className="rounded-xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/40 px-4 py-2.5 text-sm text-red-600 dark:text-red-400">{error}</div>
        )}
        ...
        {shown?.length === 0 && (
          <div className="rounded-2xl border border-dashed border-slate-300 dark:border-slate-600 bg-white/60 dark:bg-slate-900/60 px-6 py-16 text-center">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500">
              <Layers size={22} />
            </div>
        ...
        {shown?.map((run) => {
          ...
          return (
            <div key={run.id} className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
              {/* Summary row */}
              <div className={`flex flex-wrap items-center gap-3 px-4 py-3 ${open ? 'border-b border-slate-100 dark:border-slate-800' : ''}`}>
                <button onClick={() => toggle(run)} className="flex min-w-0 flex-1 items-center gap-2.5 text-left">
                  <ChevronRight size={16} className={`shrink-0 text-slate-400 dark:text-slate-500 transition ${open ? 'rotate-90' : ''}`} />
                    ...
                    {renaming === run.id ? (
                      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          className="w-64 rounded-lg border border-brand-300 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900"
                        />
                        <button onClick={() => rename(run)} className="rounded p-1 text-brand-600 dark:text-brand-400 hover:bg-brand-50 dark:hover:bg-brand-950/40"><Check size={14} /></button>
                        <button onClick={() => setRenaming(null)} className="rounded p-1 text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800"><X size={14} /></button>
                      </div>
                    ) : (
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <div className="rounded-lg bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-xs font-semibold tabular text-slate-700 dark:text-slate-200">
                  ...
                  <a
                    ...
                    className="rounded-lg border border-slate-200 dark:border-slate-700 p-1.5 text-slate-500 dark:text-slate-400 transition hover:border-brand-300 hover:text-brand-600"
                  >
                    <Download size={14} />
                  </a>
              ...
              {/* Expanded ads */}
              {open && (
                <div className="bg-slate-50/50 dark:bg-slate-900/50 p-3">
                  {bucket.loading && (
                    <div className="flex items-center gap-2 px-2 py-6 text-sm text-slate-400 dark:text-slate-500">
                      <Loader2 size={14} className="animate-spin" /> Loading ads…
                    </div>
                  )}
                  {bucket.error && (
                    <div className="px-2 py-4 text-sm text-red-600 dark:text-red-400">{bucket.error}</div>
                  )}
                  {bucket.rows && (
                    bucket.rows.length
                      ? <ResultsTable businesses={bucket.rows} compact />
                      : <div className="px-2 py-6 text-sm text-slate-400 dark:text-slate-500">This execution collected no ads.</div>
                  )}
                </div>
              )}
            </div>
  ...
  return (
    <button
      ...
      className={`rounded-lg border p-1.5 transition ${
        danger
          ? 'border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-red-300 hover:text-red-600'
...
function StatusChip({ status }) {
  ...
  const tone = status === 'error' ? 'text-red-600 dark:text-red-400' : status === 'stopped' ? 'text-amber-600' : 'text-brand-600 dark:text-brand-400';
